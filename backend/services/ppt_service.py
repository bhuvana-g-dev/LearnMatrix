...
def build_deck_sections(notes: dict) -> list[dict]:
    """Convert AI deck content into renderer-ready slide data.

    The function preserves the AI design brief so the renderer can choose
    a visual composition such as hero, timeline, cycle or architecture
    instead of relying only on the legacy text/list/process/comparison
    layout field.
    """
    slides = []
    ...
    for section in notes.get("sections", []):
        ...
        slide = {
            "kind": section.get("layout", "text"),
            "heading": heading,
            "body": section.get("content", ""),
            "design_type": section.get("design_type", "concept"),
            "visual_intent": section.get("visual_intent", ""),
            "visual_priority": section.get("visual_priority", "medium"),
            "text_density": section.get("text_density", "low"),
            "emphasis": section.get("emphasis", heading),
...
def build_pptx_from_deck_content(notes: dict, subtitle: str) -> BytesIO:
    ...
    for i, slide_data in enumerate(deck_sections):
        ...
        if design_type == "hero":
            ...
            _add_cycle_slide(
                prs, blank_layout, heading,
                slide_data.get("steps", []),
                slide_data.get("items", []),
                slide_data.get("subpoints", []),
                i + 1, accent
            )

        elif design_type in {"architecture", "application_map"}:
            _add_architecture_slide(
                prs, blank_layout, heading,
                slide_data.get("steps", []),
                slide_data.get("items", []),
                slide_data.get("subpoints", []),
        ...
        elif design_type == "problem_solution":
            _add_problem_solution_slide(
                prs, blank_layout, heading,
                slide_data.get("left", {}),
        ...
        elif design_type == "data_story":
            _add_data_story_slide(
                prs, blank_layout, heading,
                slide_data.get("items", []),
                slide_data.get("subpoints", []),
                body, i + 1, accent
            ...
            _add_bullet_slide(prs, blank_layout, heading, slide_data.get("items", []), i + 1, accent)

        elif kind == "list":
            _add_list_slide(prs, blank_layout, heading, slide_data.get("items", []), i + 1, accent)

        elif kind == "process":
            _add_process_slide(prs, blank_layout, heading, slide_data.get("steps", []), i + 1, accent)
            ...
            _add_text_slide(
                prs, blank_layout, heading, body, i + 1, accent,
                image_url, slide_data.get("subpoints")
            )
...
# ---------------------------------------------------------------------------
# AI presentation design engine
# ---------------------------------------------------------------------------

def _add_design_number(slide, index, accent):
    box = slide.shapes.add_textbox(Inches(0.7), Inches(0.45), Inches(0.9), Inches(0.4))
    tf = box.text_frame
    tf.text = f"{index:02d}"
    run = tf.paragraphs[0].runs[0]
    run.font.size = Pt(12)
...
def _add_design_heading(slide, heading, index, accent, dark=False):
    _add_design_number(slide, index, accent)
    box = slide.shapes.add_textbox(Inches(0.7), Inches(0.9), Inches(11.8), Inches(1.0))
    tf = box.text_frame
    tf.word_wrap = True
    tf.text = heading
    run = tf.paragraphs[0].runs[0]
...
def _load_slide_image(image_url):
    if not image_url:
        return None
    try:
        from services.image_service import fetch_image_bytes
...
def _safe_item_texts(steps=None, items=None, subpoints=None):
    for collection in (steps or [], items or [], subpoints or []):
        result = []
...
def _add_hero_slide(prs, layout, heading, body, index, accent, image_url=None):
    slide = prs.slides.add_slide(layout)
    ...
    circle = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(10.4), Inches(-0.9), Inches(4.0), Inches(4.0))
    _set_fill(circle, accent)
    _set_transparency(circle, 22)
    circle.shadow.inherit = False

    _add_design_number(slide, index, accent)

    title_box = slide.shapes.add_textbox(Inches(0.8), Inches(1.4), Inches(6.2), Inches(2.8))
    ...
    body_box = slide.shapes.add_textbox(Inches(0.85), Inches(4.55), Inches(5.8), Inches(1.4))
    btf = body_box.text_frame
    btf.word_wrap = True
    btf.text = body or heading
    ...
    if image_bytes:
        frame = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(7.45), Inches(1.3), Inches(4.9), Inches(5.3))
        _set_fill(frame, accent)
        frame.shadow.inherit = False
        try:
            slide.shapes.add_picture(BytesIO(image_bytes), Inches(7.55), Inches(1.4), width=Inches(4.7), height=Inches(5.1))
        except Exception:
            pass


def _add_big_statement_slide(prs, layout, heading, body, index, accent, image_url=None):
    slide = prs.slides.add_slide(layout)
    bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, prs.slide_width, prs.slide_height)
    _set_fill(bg, CREAM)
    bg.shadow.inherit = False

    block = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(0.35), prs.slide_height)
    _set_fill(block, accent)
    block.shadow.inherit = False

    _add_design_heading(slide, heading, index, accent)

    box = slide.shapes.add_textbox(Inches(1.25), Inches(2.1), Inches(6.4), Inches(3.9))
    tf = box.text_frame
    tf.word_wrap = True
    tf.vertical_anchor = MSO_ANCHOR.MIDDLE
    tf.text = body or heading
    run = tf.paragraphs[0].runs[0]
    run.font.size = Pt(28)
    run.font.bold = True
    run.font.name = HEADING_FONT
    ...
    image_bytes = _load_slide_image(image_url)
    if image_bytes:
        frame = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(8.2), Inches(1.65), Inches(4.1), Inches(4.7))
        _set_fill(frame, accent)
        frame.shadow.inherit = False
        try:
            slide.shapes.add_picture(BytesIO(image_bytes), Inches(8.32), Inches(1.77), width=Inches(3.86), height=Inches(4.46))
        except Exception:
            pass
    else:
...
def _add_timeline_slide(prs, layout, heading, steps, body, index, accent):
    ...
    labels = _safe_item_texts(steps=steps)
    if not labels:
        labels = [s.strip() for s in (body or "").split(".") if s.strip()][:5]
    if len(labels) < 2:
        _add_text_slide(prs, layout, heading, body, index, accent)
        return

    labels = labels[:6]
    start_x, end_x, line_y = Inches(1.2), Inches(12.1), Inches(3.8)
    line = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, start_x, line_y, end_x - start_x, Inches(0.07))
    _set_fill(line, NAVY_MID)
    line.shadow.inherit = False
    spacing = (end_x - start_x) / max(1, len(labels) - 1)

    for i, label in enumerate(labels):
        x = start_x + spacing * i
        node = slide.shapes.add_shape(MSO_SHAPE.OVAL, x - Inches(0.24), line_y - Inches(0.24), Inches(0.48), Inches(0.48))
        _set_fill(node, accent)
        node.shadow.inherit = False

        card_y = Inches(1.95) if i % 2 == 0 else Inches(4.35)
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, x - Inches(0.82), card_y, Inches(1.65), Inches(1.15))
        _set_fill(card, CREAM)
        card.line.color.rgb = accent
        card.shadow.inherit = False

        box = slide.shapes.add_textbox(x - Inches(0.68), card_y + Inches(0.15), Inches(1.35), Inches(0.85))
        tf = box.text_frame
        tf.word_wrap = True
        tf.vertical_anchor = MSO_ANCHOR.MIDDLE
        tf.text = label
...
def _add_cycle_slide(prs, layout, heading, steps, items, subpoints, index, accent):
    import math
    slide = prs.slides.add_slide(layout)
    _add_design_heading(slide, heading, index, accent)
    labels = _safe_item_texts(steps, items, subpoints)[:6]

    if len(labels) < 3:
        _add_text_slide(prs, layout, heading, " → ".join(labels) or heading, index, accent)
        return

    cx, cy = Inches(6.8), Inches(4.25)
    rx, ry = Inches(3.7), Inches(2.0)

    center = slide.shapes.add_shape(MSO_SHAPE.OVAL, cx - Inches(1), cy - Inches(.65), Inches(2), Inches(1.3))
    _set_fill(center, NAVY)
    center.shadow.inherit = False
    cb = slide.shapes.add_textbox(cx - Inches(.75), cy - Inches(.35), Inches(1.5), Inches(.7))
    ctf = cb.text_frame
    ctf.word_wrap = True
    ctf.text = heading
    ctf.paragraphs[0].alignment = PP_ALIGN.CENTER
    run = ctf.paragraphs[0].runs[0]
    run.font.size = Pt(14)
    run.font.bold = True
    run.font.name = HEADING_FONT
    run.font.color.rgb = WHITE
    ...
    for i, label in enumerate(labels):
        ...
        node = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, x - Inches(.85), y - Inches(.45), Inches(1.7), Inches(.9))
        _set_fill(node, accent if i % 2 == 0 else CREAM)
        if i % 2:
            node.line.color.rgb = accent
        node.shadow.inherit = False
        box = slide.shapes.add_textbox(x - Inches(.7), y - Inches(.3), Inches(1.4), Inches(.6))
        tf = box.text_frame
        tf.word_wrap = True
        tf.text = label
        tf.paragraphs[0].alignment = PP_ALIGN.CENTER
...
def _add_architecture_slide(prs, layout, heading, steps, items, subpoints, body, index, accent):
    slide = prs.slides.add_slide(layout)
    _add_design_heading(slide, heading, index, accent)
    labels = _safe_item_texts(steps, items, subpoints)[:6]
    if len(labels) < 2:
        labels = [s.strip() for s in (body or "").split(".") if s.strip()][:4]
    if len(labels) < 2:
        _add_text_slide(prs, layout, heading, body, index, accent)
        return

    cx, cy = Inches(5.15), Inches(3.45)
    center = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, cx, cy, Inches(3.0), Inches(1.0))
    _set_fill(center, NAVY)
    center.shadow.inherit = False
    box = slide.shapes.add_textbox(cx + Inches(.2), cy + Inches(.2), Inches(2.6), Inches(.6))
    tf = box.text_frame
    tf.word_wrap = True
    tf.text = heading
    tf.paragraphs[0].alignment = PP_ALIGN.CENTER
    ...
    positions = [(1.0, 2.0), (4.8, 1.85), (9.0, 2.0), (1.0, 5.1), (4.8, 5.35), (9.0, 5.1)]
    for i, label in enumerate(labels):
        px, py = positions[i]
        x, y = Inches(px), Inches(py)
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, x, y, Inches(2.45), Inches(.85))
        _set_fill(card, CREAM)
        card.line.color.rgb = accent
        card.shadow.inherit = False
        tbox = slide.shapes.add_textbox(x + Inches(.15), y + Inches(.12), Inches(2.15), Inches(.6))
        ttf = tbox.text_frame
        ttf.word_wrap = True
        ttf.vertical_anchor = MSO_ANCHOR.MIDDLE
        ttf.text = label
        ttf.paragraphs[0].alignment = PP_ALIGN.CENTER
        run = ttf.paragraphs[0].runs[0]
        run.font.size = Pt(11)
        run.font.bold = True
        run.font.name = BODY_FONT
        run.font.color.rgb = NAVY
...
def _add_problem_solution_slide(prs, layout, heading, left, right, body, index, accent):
    slide = prs.slides.add_slide(layout)
    _add_design_heading(slide, heading, index, accent)

    left_label = left.get("label", "Problem")
    right_label = right.get("label", "Solution")
    left_items = left.get("items", []) or ([body] if body else [])
    right_items = right.get("items", []) or ["Apply the key concept effectively"]

    panels = [
        (Inches(1.0), NAVY, left_label, left_items, WHITE),
        (Inches(7.05), accent, right_label, right_items, NAVY),
    ]
    for x, color, label, values, text_color in panels:
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, x, Inches(2.05), Inches(5.25), Inches(4.65))
        _set_fill(card, color)
        card.shadow.inherit = False
        box = slide.shapes.add_textbox(x + Inches(.4), Inches(2.45), Inches(4.45), Inches(.7))
        tf = box.text_frame
        tf.text = label
        run = tf.paragraphs[0].runs[0]
        run.font.size = Pt(24)
        ...
        for value in values[:4]:
            item = slide.shapes.add_textbox(x + Inches(.45), top, Inches(4.3), Inches(.65))
            itf = item.text_frame
            itf.word_wrap = True
            itf.text = f"• {value}"
            run = itf.paragraphs[0].runs[0]
            run.font.size = Pt(15)
            run.font.name = BODY_FONT
            run.font.color.rgb = text_color
            top += Inches(.78)
...
def _add_data_story_slide(prs, layout, heading, items, subpoints, body, index, accent):
    slide = prs.slides.add_slide(layout)
    _add_design_heading(slide, heading, index, accent)
    labels = _safe_item_texts(items=items, subpoints=subpoints)[:5]
    if not labels:
        labels = [s.strip() for s in (body or "").split(".") if s.strip()][:5] or [heading]

    top = Inches(2.0)
    for i, label in enumerate(labels):
        box = slide.shapes.add_textbox(Inches(1.1), top, Inches(3.0), Inches(.55))
        tf = box.text_frame
        tf.word_wrap = True
        tf.text = label
        run = tf.paragraphs[0].runs[0]
        ...
        bar = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(4.2), top + Inches(.04), Inches(4.2 + (i % 3) * .9), Inches(.48))
        _set_fill(bar, accent if i % 2 == 0 else NAVY)
        bar.shadow.inherit = False
        top += Inches(.95)

    insight = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(9.1), Inches(2.05), Inches(2.85), Inches(3.8))
    _set_fill(insight, CREAM)
    insight.line.color.rgb = accent
    insight.shadow.inherit = False
    box = slide.shapes.add_textbox(Inches(9.4), Inches(2.45), Inches(2.25), Inches(2.9))
    tf = box.text_frame
    tf.word_wrap = True
    tf.vertical_anchor = MSO_ANCHOR.MIDDLE
    tf.text = body or heading
