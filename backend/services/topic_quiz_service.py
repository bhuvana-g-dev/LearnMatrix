...
def get_topic_quiz(skill: str, topic: str) -> dict:
    """
    Hybrid sourcing, same priority order as services/learning_content_service.py
    uses for resources: admin-curated Question Bank first, AI fills only
    the shortfall. Bank questions and AI questions are returned in the
    SAME shape (QuestionID for bank rows, TempID for AI rows) so the
    frontend doesn't need to know which source each came from — it only
    matters again at submit time (see _lookup_correct_answer below).
    """
    ...
    if shortfall > 0:
        try:
            ...
            if not bank_questions:
                ...
                # not a "smaller quiz than usual" situation.
