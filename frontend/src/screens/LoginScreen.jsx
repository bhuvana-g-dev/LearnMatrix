...
/**
 * LoginScreen is now purely presentational: it receives `auth` (the object
 * returned by useAuth()) and an `onSuccess` callback. It doesn't know or
 * care whether login is dummy or backed by Flask/Firebase.
 */
export default function LoginScreen({ auth, onSuccess }) {
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between text-xs pt-1">
              <label className="flex items-center gap-2 cursor-pointer select-none" style={{ color: COLORS.textMid }}>
                <span
                  ...
                  style={{
                    ...
                    height: 16,
                    borderRadius: 5,
                    background: remember ? GRADIENTS.purplePink : "rgba(255,255,255,0.6)",
                    border: remember ? "none" : `1px solid ${COLORS.textLight}`,
            ...
            {auth.error && (
              <p className="text-center text-xs" style={{ color: "#E4568A" }}>
                {auth.error}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <motion.button
              ...
              style={{ padding: "10px 0", borderRadius: 9999, border: `1px solid ${COLORS.border}`, background: "rgba(255,255,255,0.45)", color: COLORS.textDark, cursor: "pointer" }}
            >
              <GoogleIcon /> Google
            </motion.button>
            <motion.button
          ...
          <p className="text-center text-xs mt-7" style={{ color: COLORS.textMid }}>
            Don't have an account?{" "}
            <a href="#" style={{ color: "#8B5CF6", fontWeight: 700 }}>
              Sign Up
            </a>
          </p>
        </motion.div>
